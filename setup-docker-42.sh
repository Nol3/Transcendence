...
# Check if using Rootless Docker
print_header "3.1 Verificando configuración de Docker"
...
if docker info 2>/dev/null | grep -q "rootless"; then
    ...
    print_success "Docker en modo estándar"
fi
...
NGINX_URL="https://localhost:$NGINX_PORT"
...
if [ "$NGINX_PORT" = "443" ]; then
    ...
    print_success "Nginx (HTTPS): https://localhost:$NGINX_PORT"
    print_success "Nginx (HTTP): http://localhost:$HTTP_PORT"
fi
